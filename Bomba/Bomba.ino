...
void loop() {
  if ((bombSet && isTimerStarted) && (!exploded && !defused)) {  //This condition statement is written for controlling the timers.
    ...
    if (key && isDigit(key)) {
      for (i = 0; i <= trypswdLen; i++) {
        trypswd[i] = NULL;
      }                  //If bomb set and a key is pressed, whether the key is a digit. This condition is activated.
...
void bombdefuse() {  //It's continue of 'defusing the bomb' (the starting 'if statement' is in loop function)
  ...
  for (i = 0; i <= trypswdLen; i++) {
    if (pswd[i] == trypswd[i] && i == trypswdLen - 1) {  //Here, 'pswd'(it's received before the bomb set) and 'trypswd'(it's received after the bomb set) are comparing. If both are same bomb will be defused
      defused = true;
      timer.cancel(taskBuzzer);
      timer.cancel(bombTimer);
      lcd.clear();
